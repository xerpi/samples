...
int main(int argc, char *argv[])
{
	...
	SceUID triangle_mesh_uid;
	struct basic_vertex *const triangle_mesh_data = gpu_alloc_map(
		SCE_KERNEL_MEMBLOCK_TYPE_USER_RW_UNCACHE, SCE_GXM_MEMORY_ATTRIB_READ,
		3 * sizeof(struct basic_vertex), &triangle_mesh_uid);

	SceUID triangle_indices_uid;
	unsigned short *const triangle_indices_data = gpu_alloc_map(
		SCE_KERNEL_MEMBLOCK_TYPE_USER_RW_UNCACHE, SCE_GXM_MEMORY_ATTRIB_READ,
		3 * sizeof(unsigned short), &triangle_indices_uid);
	...
	triangle_indices_data[0] = 0;
	triangle_indices_data[1] = 1;
	triangle_indices_data[2] = 2;
	...
	while (run) {
		 ...
		 */
		{
			...
			sceGxmSetVertexStream(gxm_context, 0, triangle_mesh_data);
			sceGxmDraw(gxm_context, SCE_GXM_PRIMITIVE_TRIANGLES,
				SCE_GXM_INDEX_FORMAT_U16, triangle_indices_data, 3);
		}
	...
	gpu_unmap_free(triangle_mesh_uid);
	gpu_unmap_free(triangle_indices_uid);
